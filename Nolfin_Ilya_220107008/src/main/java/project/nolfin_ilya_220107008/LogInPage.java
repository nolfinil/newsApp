...
public class LogInPage {
    ...
    boolean checkData(){
        ...
        if(new File("cargo/"+loginField.getText()+".txt").exists()) {
            try (
            ...
            errorText.setText("User not exist");
        }
...
//       MouseEvents Hover object
        ...
        loginPageSignUpButton.setOnMouseMoved(event -> {
            loginPageSignUpButton.setTextFill(Color.BLACK);
        });
        loginPageSignUpButton.setOnMouseExited(event -> {
            loginPageSignUpButton.setTextFill(Color.WHITE);
        });
